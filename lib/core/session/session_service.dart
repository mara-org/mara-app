...
class SessionService {
  ...
  Future<Map<String, dynamic>> createBackendSession() async {
    ...
    if (refreshedUser == null || !refreshedUser.emailVerified) {
      debugPrint(
          '❌ SessionService: Email not verified - cannot create backend session');
      throw Exception(
          'Email not verified. Please verify your email address before accessing the app.');
    }
    ...
    try {
      ...
      debugPrint(
          '📤 SessionService: device_id type: ${deviceId.runtimeType}, value: $deviceId');
      debugPrint(
          '📤 SessionService: device_name type: ${deviceName.runtimeType}, value: $deviceName');
      ...
      if (response.containsKey('error')) {
        ...
        debugPrint(
            '❌ SessionService: Backend returned error: ${response['error']}');
      ...
      final backendToken =
          response['token'] as String? ?? response['session_token'] as String?;
      if (backendToken != null && backendToken.isNotEmpty) {
        await _secureStore.write(
            key: 'backend_session_token', value: backendToken);
      }
      ...
      if (errorString.contains('correlation_id') ||
          errorString.contains('Backend')) {
        debugPrint(
            '❌ SessionService: Backend error details found in exception');
      }
  ...
  Future<Map<String, dynamic>> fetchUserInfo() async {
    ...
    if (response.containsKey('error')) {
      debugPrint(
          '❌ SessionService: Backend returned error: ${response['error']}');
      throw Exception(
          response['error']['message'] ?? 'Failed to fetch user info');
    }
